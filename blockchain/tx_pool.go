...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	if uint64(pool.all.Slots()+numSlots(tx)) > pool.config.ExecSlotsAll+pool.config.NonExecSlotsAll {
		...
		// (3) discard underpriced transactions. Decided before the eviction in
		// (4); the previous evict-then-check order destroyed maxTx on every
		// refusal and deleted single-tx queue entries with it.
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	if queued > pool.config.NonExecSlotsAll {
		// Sort all accounts with queued transactions by heartbeat, stalest last.
		// Without sort.Reverse the loop below evicted the freshest accounts first.
