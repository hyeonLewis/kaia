...
func (p *StateProcessor) FinalizeState(header *types.Header, statedb *state.StateDB, txs []*types.Transaction, receipts types.Receipts) (*types.Block, error) {
	...
	// txs and receipts are immutable past the apply loop, so their roots and the
	// bloom can be derived concurrently with the state-root hashing below.
	var (
	...
	go func() {
		defer close(derived)
		if len(txs) > 0 {
			txRoot = types.DeriveTransactionsRoot(txs, header.Number)
		}
		if len(receipts) > 0 {
			receiptRoot = types.DeriveReceiptsRoot(receipts, header.Number)
