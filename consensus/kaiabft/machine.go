...
func (m *machine) startNewRound(round *big.Int) {
	...
	if roundChange && m.isProposer() {
		if m.isHashLocked() && m.preprepare != nil {
			...
			m.b.eventMux.Post(consensus.NewSequenceEvent{RoundChange: true, IsProposer: true})
		}
	}
...
func (m *machine) sendPreprepare(request *bft.Request) {
	...
	// the proposer pays the decode + verify on its own block).
	if m.state == stateAcceptRequest && !m.isHashLocked() {
